...
includeWithVariables('./layout/header.php', array('webTitle' => 'Data Tilang - MELAS'));
...
<main>
  <div class="title-bar d-flex justify-content-center">
    <h2 class="text-white font-weight-bold">DATA TILANG</h2>
  </div>
  ...
  <section class="content d-flex flex-column">
    ...
    <div class="container respon-pengawalan">
      <div class="float-right tambah-berita">
        <a href="tambah_data_tilang.php"><i class="fa fa-plus"></i></a>
      </div>
      <table id="table_tilang" class="table tbl_respon text-center table-striped table-bordered bg-white" style="width:100%">
        <thead>
          <tr>
            <th>NO TILANG</th>
            <th>NO BLANKO</th>
        ...
        <tbody>
          <?php
          $data = mysqli_query($koneksi, "SELECT * from tbl_tilang");
          while ($result = mysqli_fetch_array($data)) {
            $no_tilang = $result['no_tilang'];
            $badge_color = '';
            if ($result['status_denda'] == 'Belum Dibayar') {
              $badge_color = 'badge-danger';
            } else if ($result['status_denda'] == 'Sudah Dibayar') {
              $badge_color = 'badge-success';
            }

            echo "<tr class='clickable-row' data-href='detail_tilang.php?no=$no_tilang'>";
            echo "<td>" . $result['no_tilang'] . "</td>";
            echo "<td>" . $result['no_blanko'] . "</td>";
            echo "<td>" . $result['nama_pelanggar'] . "</td>";
            echo "<td>" . $result['tgl_pelanggaran'] . "</td>";
            echo "<td>" . $result['no_polisi'] . "</td>";
            echo "<td><span class='badge $badge_color'>" . $result['status_denda'] . "</span></td>";

            // echo "<td> 
            // <a class='btnEditBerita' ><i class='fas fa-edit text-warning' style='cursor:pointer;'></i></a> 

            // <a class='btnHapusBerita' style='cursor:pointer;'><i class='fas fa-trash text-danger'></i></a>
            // </td>";
            echo "</tr>";
          } ?>
        </tbody>
